...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='configs/pool_random.yaml', help='path to config file')
    ...
    # set up tensorboard
    select_name = f'ratio{config.strategy.select_ratio}_seed{args.random_seed}'
    ...
    for epoch in range(config.train.n_epochs):
        ...
        if epoch % config.strategy.select_freq == 0:
            ...
            existing_select_indices = train_coreset.selected_indices
            if epoch == 0:
                unselected_indices = range(len(train_dataset))
            else:
                unselected_indices = [i for i in range(len(train_dataset)) if i not in existing_select_indices]
            random_indices = np.random.choice(unselected_indices, each_iter_select_cnt, replace=False)
            if epoch == 0:
                selected_indices = random_indices
            else:
                selected_indices = np.concatenate((existing_select_indices, random_indices))
            train_coreset.set_indices(selected_indices)
            logger.info(f'random_indices: {len(random_indices)} {random_indices}, current selected_indices: {len(train_coreset.selected_indices)} {train_coreset.selected_indices}')
